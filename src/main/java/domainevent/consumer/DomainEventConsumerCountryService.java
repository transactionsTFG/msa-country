...
@MessageDriven(mappedName = JMSQueueNames.AGENCY_COUNTRY_SERVICE_QUEUE)
    ...
    public void onMessage(Message msg) {
        try {
            if(msg instanceof TextMessage m) {
                ...
                LOGGER.info("Recibido en Cola {}, Evento Id: {}, EventResponse: {}", JMSQueueNames.AGENCY_COUNTRY_SERVICE_QUEUE, event.getEventId(), event.getValue());
                EventHandler commandHandler = this.eventHandlerRegistry.getHandler(event.getEventId());
                if(commandHandler != null)
                    commandHandler.handleCommand(gson.toJson(event.getValue()));
            }
        } catch (Exception e) {
